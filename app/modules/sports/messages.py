...
class Game(messages.Message):
	...
	players_full = messages.BooleanField(1, required=True)
	...
	players_joined = messages.IntegerField(6, required=True)
	lat = messages.FloatField(7, required=False)
	lon = messages.FloatField(8, required=False)
	categories_full = messages.MessageField(SportCategory, 9, repeated=True)
	end_time = message_types.DateTimeField(10, required=False)
	location_name = messages.StringField(11, required=False)
	creator_id = messages.IntegerField(12, required=True)
	players = messages.MessageField(User, 13, repeated=True)
